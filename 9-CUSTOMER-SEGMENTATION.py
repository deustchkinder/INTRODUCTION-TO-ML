...
#GOREV:
#K-Means'i kullanarak müşteri segmentasyonu yapağız.
#Müşterinin kaç gün önce alışveriş yaptığı, ne sıklıkla alışveriş yaptığı ve toplam harcadığı miktar gibi yeni değişkenler oluşturarak ilerleyebiliriz.
...
df_ = pd.read_excel ("C:/Users/Monster/Desktop/MACHINE LEARNING SUMMER CAMP/3-machine_learning/odevler2/online_retail_II.xlsx", sheet_name="Year 2009-2010")
df = df_.copy ()
...
#LOG TRANSFORMATION
for col in["Receny","Frequency"]:
    rfm[f"LOG_{col}"] = np.logip(rfm[col])
rfm.head()

#SCALER
sc=StandardScaler()
sc.fit(rfm[["LOG_Recency","LOG_Frequency"]])
scaled_rf = sc.transform(rfm[["LOG_Receny","LOG_Frequency"]])
